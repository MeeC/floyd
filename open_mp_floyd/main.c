...
{
	int nodes_num=500,max_distance=200,num_of_threads=4,graph_type=RANDOM,proc_mode=OMP_PAR,print_mode=0,policy=1;
	char * filename;
	...
	if(argc==5)	// user provides arguments
	{
		...
		policy=atoi(argv[4]);	// user_given number of threads		
	...
	else if(proc_mode==OMP_PAR)openMP_floyd_algorithm(nodes_num,policy);
